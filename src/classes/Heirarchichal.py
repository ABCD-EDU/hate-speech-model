...
class HeirarchichalNN(pl.LightningModule):
    def __init__(self, task1_n_classes: int = 3, task2_n_classes: int = 1, task3_n_classes: int = 5,  n_training_steps=None, n_warmup_steps=None, bert_model_name='distilbert-base-uncased'):
        super().__init__()
        self.bert = AutoModel.from_pretrained(
            bert_model_name, return_dict=True)
        self.hidden = nn.Linear(self.bert.config.hidden_size,
                                self.bert.config.hidden_size)
        self.hidden1 = nn.Linear(task1_n_classes, self.bert.config.hidden_size)
        self.hidden2 = nn.Linear(task2_n_classes, self.bert.config.hidden_size)
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        output1_ = self.hidden1(output1)
        output1_ = self.dropout(output1_)
        output1_ = F.relu(output1_)

        output2 = self.task2_classifier(output1_)
        output2_ = self.hidden2(output2)
        output2_ = self.dropout(output2_)
        output2_ = F.relu(output2_)

        output3 = self.task3_classifier(output2_)

        # output1 = torch.sigmoid(output1)
        # Add softmax on each batch
        ...
        if labels is not None:
            ...
            # loss = loss1*.5 + loss2*.25 + loss3*.25
